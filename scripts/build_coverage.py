"""収集状況 coverage.json を生成する.

母集合 (data/pokemon.json = Champions 実装ポケモン名の配列) と収集済み
(data/collected.json = {ポケモン名: [技名...]}) を突合し、各ポケモンの収集状況を
site/coverage.json に出力する。GitHub Pages のステータスページがこれを読んで可視化する。

収集キーはポケモン名。技プールが個別に異なるフォーム (地方フォーム等) は
別名で母集合に含む (技プールがベースと共通のメガ・サイズ違い等は母集合から除く)。
「収集済み」の基準 (is_collected / MIN_MOVES) は _data.py に集約してある。
