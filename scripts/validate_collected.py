...
def _validate_master(names: object) -> list:
    """収録対象一覧 pokemon.json 自身の健全性を検証する (収録側と非対称にしない)."""
...
def main() -> int:
    # 収録対象一覧自体を先に検証する (収録対象一覧が壊れていれば収録側の検証は無意味)
    names_list = _data.load_pokemon()
    master_errors = _validate_master(names_list)
    if master_errors:
        print(f"収録対象一覧 pokemon.json の検証失敗 ({len(master_errors)} 件):")
    ...
    print(
        f"検証OK: 収録対象一覧 {len(names)} 種 / 収録 {len(collected)} 種、"
        "全キーが収録対象一覧と一致"
    )
    return 0
